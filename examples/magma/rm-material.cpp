...
int main(void)
{
    ...
    for (auto i = 0u; i < SPHERES_SIDE_COUNT; ++i) {
        for (auto j = 0u; j < SPHERES_SIDE_COUNT; ++j) {
            ...
            material.roughness(i / static_cast<float>(SPHERES_SIDE_COUNT - 1u));
            material.metallic(j / static_cast<float>(SPHERES_SIDE_COUNT - 1u));
    ...
    // Move camera
    app.camera().position({-7.f, -5.f, 10.f});
    app.camera().target({SPHERES_SIDE_COUNT * 0.65f, SPHERES_SIDE_COUNT * 0.65f, 0.f});
