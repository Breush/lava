...
#include <lava/crater/Context.hpp>
#include <lava/chamber/ThreadLocalPtr.hpp>
#include <lava/chamber/Mutex.hpp>
#include <lava/chamber/Lock.hpp>
#include <lava/chamber/Err.hpp>
#include <lava/OpenGL.hpp>
#include <algorithm>
#include <vector>
#include <string>
#include <set>
        ...
        if ((m_settings.majorVersion == 3) && (m_settings.minorVersion == 1))
        {
            ...
            if (glGetStringiFunc)
            {
                ...
                for (unsigned int i = 0; i < static_cast<unsigned int>(numExtensions); ++i)
                {
                    ...
                    {
                        m_settings.attributeFlags &= ~static_cast<uint32_t>(ContextSettings::Core);
                        break;
                    }
                }
            }
