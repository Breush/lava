...
{
    logger.info("flow.wave-sound-reader") << "Reading '" << fileName << "'." << std::endl;
    logger.log().tab(1);

    std::ifstream file(fileName, std::ifstream::binary);

    if (!file.is_open()) {
        logger.error("flow.wave-sound-reader") << "File cannot be open." << std::endl;
    }
    ...
    if ((m_fileData.size() < 44u)
        ...
        logger.error("flow.wave-sound-reader") << "File is not a Wave one." << std::endl;
    }

    // Check format
    auto audioFormat = readUint16LE(m_fileData.data(), 20);
    if (audioFormat != 1u) {
        logger.error("flow.wave-sound-reader") << "Unknown audio format " << audioFormat << "." << std::endl;
    }
    ...
    case 8u: m_soundFormat = SoundFormat::Uint8; break;
    case 16u: m_soundFormat = SoundFormat::Int16LE; break;
    case 32u: m_soundFormat = SoundFormat::Int32LE; break;
    default: logger.error("flow.wave-sound-reader") << "Unknown bitsPerSample value." << std::endl;
    }

    logger.log() << "Channels: " << static_cast<uint32_t>(m_channels) << std::endl;
    logger.log() << "Rate: " << m_rate << std::endl;
    logger.log() << "Sound format: " << m_soundFormat << std::endl;
